...
The View/Print Criteria Set form allows existing criteria sets to be reviewed and printed. To bring up the form, select the Trainer/Supervisor - View/Print Criteria Set menu item on the [main form](broken-id-3znysh7.md).
...
The Trainer/Supervisor form Write Evaluation tab is accessed via the Trainer/Supervisor - Write Evaluation menu item on the [main form](broken-id-3znysh7.md).

![](<.gitbook/assets/2 (1).jpeg>)

The Write Evaluation tab displays a list of trainees/employees for which the current user can write an evaluation. This may include all trainees/employees if the current user is assigned to ['Department ALL'](broken-id-37m2jsg.md). An asterisk (\*) denotes that one or more incomplete evaluations exist for the trainee. The evaluation criteria set is shown in the drop down selector below the Trainee/Employee list.

![](<.gitbook/assets/3 (1).png>)

To write an evaluation, first select a trainee/employee. This done, the default criteria set is automatically entered, any incomplete evaluations are displayed, and the Write Evaluation button is enabled. The criteria set can be changed at this time, if desired, by making another selection from the drop down list. Clicking on the Write Evaluation button without selecting an evaluation opens the [Evaluation Writer](broken-id-sqyw64.md) with a new evaluation. To edit an existing evaluation, first make a selection from the Incomplete Evaluations list. The list entries are sequentially numbered and are sorted by the evaluation end date. Once an evaluation is selected, the criteria set is automatically entered and cannot be changed.
...
The Trainer/Supervisor form Review Evaluation tab is accessed via the Trainer/Supervisor - Review Evaluation menu item on the [main form](broken-id-3znysh7.md).

![](broken-id-2dlolyb.md), the [Trainee/Employee Evaluation tab](broken-id-3cqmetx.md), and the Reviewer Comments tab. Operation of the first two tabs is the same as on the [Evaluation Viewer](broken-id-2dlolyb.md). They are used by the reviewer to look over the evaluation. After the evaluation has been checked, any comments the reviewer might have are entered on the Reviewer Comments tab. It includes the "Comments" (read-only) and "New" (editable) textboxes, as well as options for approving and disapproving the evaluation. Use the Spelling button to spell check the comments before submitting. Select the Submit button after the comments have been entered and the approval state has been set:

![](broken-id-1rvwp1q.md) when it is sent back for correcting, the Evaluation Viewer once approved by all reviewers, and in the evaluation report preview and print-off when configured to do so from the [Reviewer Comments](broken-id-4bvk7pj.md) sub-tab.

## NOTE: Until all reviewers have approved the evaluation, or unless turned off in the [Unapproved Evaluations](broken-id-2r0uhxc.md) configuration sub-tab of the Global Settings tab, DRAFT will appear across each page of the report when printing or previewing.

***

The Signatures tab will appear in the Evaluation Reviewer to allow an evaluation to be signed by the reviewer when the "Allow completed evaluations to be signed before they are fully-reviewed" checkbox is checked (in the [Signatures](broken-id-1664s55.md) sub-tab of the [System Utilities](broken-id-2xcytpi.md) form's [Global Settings](broken-id-46r0co2.md) tab).
...
The Trainer/Supervisor form Reports tab is accessed via the Trainer/Supervisor - Reports menu item on the [main form](broken-id-3znysh7.md).

![](broken-id-3q5sasy.md)
* [Trainee/Employee Roster](broken-id-25b2l0r.md)
* [Trainer/Supervisor Reports](broken-id-kgcv8k.md)
* [Trainee/Employee Reports](broken-id-34g0dwd.md)
* [Training Time Report](broken-id-1jlao46.md)
...
The Download form allows information entered at one workstation to be saved to an external file for the purpose of transferring to another workstation. It provides users that do not have network access the ability to maintain a [centralized database](broken-id-3dy6vkm.md). To bring up the Download form select the Administrator - Download menu item on the [main form](broken-id-3znysh7.md).

![](broken-id-37m2jsg.md) and the user's [security level](broken-id-3ygebqi.md). The tabs are labeled as follows:

* [Trainers/Supervisors](broken-id-206ipza.md)
* [Trainees/Employees](broken-id-4k668n3.md)
* [Evaluations by Trainer/Supervisor](broken-id-43ky6rz.md)
* [Evaluations by Trainee/Employee](broken-id-1egqt2p.md)
* [Criteria Sets](broken-id-2zbgiuw.md)
* [System Tables](broken-id-3l18frh.md)
...
The Upload form allows information entered and downloaded at one workstation to be transferred to another. It provides users that do not have network access the ability to maintain a [centralized database](broken-id-3dy6vkm.md). To bring up the Upload form select the Administrator - Upload menu item on the [main form](broken-id-3znysh7.md).

![](broken-id-37m2jsg.md) and by the user's [security level](broken-id-3ygebqi.md). Security levels 1 and 2 are only allowed to upload evaluations. On a stand-alone system only trainers/supervisors, trainees/employees and criteria sets can be uploaded.
